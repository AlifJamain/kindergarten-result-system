...
if(!empty($_POST["classid1"])) 
{
 ...
 $status=0;	
 $stmt = $dbh->prepare("SELECT tblsubjek.nama_subjek,tblsubjek.id FROM tblgabung join tblsubjek on  tblsubjek.id=tblgabung.id_subjek WHERE tblgabung.id_kelas=:cid and tblgabung.status!=:stts");
 $stmt->execute(array(':cid' => $cid1,':stts' => $status));
 
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
      <div class="form-group">
          <label class="col-md-4 control-label"><?php echo htmlentities($row['nama_subjek']); ?></label>
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT id_subjek,id_kelas FROM tblkeputusan WHERE id_pelajar=:id1 and id_kelas=:id ");
