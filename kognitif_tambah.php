...
$mark=$_POST['marks'];
 ...
 $stmt = $dbh->prepare("SELECT tblsubjek.nama_subjek,tblsubjek.id FROM tblgabung join  tblsubjek on  tblsubjek.id=tblgabung.id_subjek WHERE tblgabung.id_kelas=:cid order by tblsubjek.nama_subjek");
...
for($i=0;$i<count($mark);$i++){
    $mar=$mark[$i];
  $sid=$sid1[$i];
$sql="INSERT INTO  tblkeputusan(id_pelajar,id_kelas,id_subjek,marks, empid) VALUES(:studentid,:class,:sid,:marks, :empid)";
...
<?php }} ?>
                ...
                <div class="form-group">
            <button class="btn btn-primary" type="submit" name="add"onclick="myFunction()">Tambah</button>&nbsp;<a href='kognitif_papar.php' class='btn btn-danger'>Balik</a>
    <script>
function myFunction() {
